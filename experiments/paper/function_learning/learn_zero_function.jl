# Sanity Check: Learn the Zero Function
# This script attempts to learn f(x) such that all outputs are 0
# This should be easy since the trivial solution is f(x) = 0
...
println("="^80)
println("SANITY CHECK: Learning the Zero Function")
println("="^80)
println()
...
PREC = 20
PRIME = 3
N_POINTS = 6  # Number of random points
POLY_DEGREE = 12  # Polynomial degree
EPOCHS = 500
...
for i in 1:N_POINTS
    ...
    # Target is always 0 (zero function)
    y_val = 0.0

    push!(data_points, (x=x_val, y=y_val))
end

println("Data points (all targets = 0):")
...
fun = AbsolutePolynomialSum([poly_expr])
...
param = ValuationPolydisc(param_center, [INITIAL_RADIUS for _ in 1:POLY_DEGREE+1])
...
optim = greedy_descent_init(param, loss, 1, settings)
...
if accuracy == 100.0
    println()
    println("SUCCESS! Achieved 100% accuracy on zero function learning.")
    println()
    ...
    for (i, coef) in enumerate(param_center)
        ...
        if degree == 0
            println("  a0 (constant): $coef, |a0|_p = $coef_norm")
        else
            println("  a$degree (x^$degree): $coef, |a$degree|_p = $coef_norm")
        end
    end
else
    println()
